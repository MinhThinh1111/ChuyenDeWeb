...
import { addServiceCallAPI } from '../../services/admin/KietService';
import QuanLyGoiDangKy from './QuanLyGoiDangKy';

class AddDichvu extends React.Component {
    state = {
        id: "",
        thoiHan: "",
        soLuongPhongToiDa: "",
    ...
    async themDichVu() {
        if (window.confirm("Xác nhận thêm dịch vụ")) {
            if (this.kiemTraRong()) {
                let res = await addServiceCallAPI(this.state.thoiHan, this.state.soLuongPhongToiDa, this.state.gia);
                if (res != null) {
                                            ...
                                            <div>
                                                <div>
                                                    <h5 className="card-title mb-0">
                                                        <button className="btn btn-warning" onClick={() => this.themDichVu()}>Thêm dịch vụ</button>
